    ...
    <div className="max-w-6xl mx-auto px-6 py-10 pb-32 animate-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="relative">
          ...
          <p className="text-xs text-zinc-500 font-bold uppercase tracking-[0.2em] mt-1 ml-6">Cyber Oversight • Live Link</p>
        </div>
        <button onClick={onLogout} className="px-6 py-2.5 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-xl text-[10px] font-black uppercase transition-all hover:bg-rose-500 hover:text-white tracking-widest shadow-lg">Terminate Access</button>
      </div>
      ...
      <div className="space-y-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in">
                     ...
                     <div key={u.id} className="p-5 bg-zinc-950/40 rounded-2xl border border-white/5 flex items-center justify-between group hover:border-emerald-500/20 transition-all">
                       <div className="flex items-center gap-4">
                         ...
                         <div>
                           ...
                           <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest">{u.role === 'ADMIN' ? 'SYS ADMIN' : u.uid}</p>
                         </div>
                       </div>
                       <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                         <button onClick={() => {
                           const tag = prompt('Assign Protocol Label:');
                           if(tag) onUpdateProfile(u.id, { tags: [...u.tags, tag.toUpperCase()] });
                         }} className="p-2 text-emerald-500 bg-emerald-500/5 rounded-lg border border-emerald-500/10 hover:bg-emerald-500 hover:text-black transition-all" title="Label Personnel">
                         ...
                         }} className="p-2 text-rose-500 bg-rose-500/5 rounded-lg border border-rose-500/10 hover:bg-rose-500 hover:text-white transition-all" title="Erase Identity">
             ...
             <div className="lg:col-span-4 space-y-6">
               <div className="premium-card p-8 bg-emerald-500/[0.02]">
                 <h3 className="text-[10px] font-black text-zinc-500 uppercase mb-8 tracking-[0.2em]">Isolation Queue</h3>
                 <div className="max-h-[500px] overflow-y-auto pr-2 space-y-4">
                   {leaves.filter(l => l.status === 'PENDING').length === 0 ? <p className="text-center py-10 text-zinc-800 text-[10px] font-black uppercase">No active requests</p> : leaves.filter(l => l.status === 'PENDING').map(l => (
        ...
        {activeTab === 'REPORTS' && (
          <div className="max-w-2xl mx-auto space-y-6 animate-in">
            <h3 className="text-xs font-black text-center text-zinc-500 uppercase tracking-widest mb-10">Neural Work Log registry</h3>
            {reports.length === 0 ? <p className="text-center py-20 text-zinc-800 font-black uppercase text-xs">Registry Empty</p> : reports.slice().reverse().map(r => (
               <div key={r.id} className={`premium-card p-8 border-l-4 space-y-4 transition-all ${r.status === 'APPROVED' ? 'border-emerald-500 bg-emerald-500/[0.02]' : r.status === 'REJECTED' ? 'border-rose-500 bg-rose-500/[0.02]' : 'border-indigo-500 bg-indigo-500/[0.02]'}`}>
                  ...
                  {r.attachment && (
                    <div className="p-3 bg-zinc-950 rounded-xl border border-white/5 flex items-center justify-between">
                       <p className="text-[10px] text-zinc-600 font-black uppercase">Encrypted Evidence Captured</p>
                       <button onClick={() => window.open(r.attachment)} className="text-[9px] text-emerald-500 font-black uppercase hover:underline">Download Proof</button>
                    </div>
                  )}
                  ...
                  <div className="flex gap-3 pt-2">
                    {r.status === ReportStatus.PENDING && (
                      <>
                        <button onClick={() => onReviewReport(r.id, ReportStatus.APPROVED)} className="px-5 py-2 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-black transition-all">Accept Log</button>
                        <button onClick={() => onReviewReport(r.id, ReportStatus.REJECTED)} className="px-5 py-2 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-rose-500 hover:text-white transition-all">Void Log</button>
                      </>
                    )}
        ...
        {activeTab === 'TASKS' && (
          <div className="max-w-md mx-auto animate-in">
            <form onSubmit={(e) => {
              ...
              <h3 className="text-xl font-black text-center mb-6 text-white uppercase tracking-tighter">Issue Personal Directive</h3>
              <div className="space-y-4">
                <div className="space-y-1">
                   <label className="text-[9px] font-black text-zinc-600 uppercase ml-1">Directive Label</label>
                   <input name="title" required placeholder="Subject..." className="w-full bg-zinc-950 border border-white/10 p-4 rounded-xl text-sm outline-none focus:border-indigo-500 transition-all text-white placeholder-zinc-800" />
                </div>
                <div className="space-y-1">
                   <label className="text-[9px] font-black text-zinc-600 uppercase ml-1">Target Personnel</label>
                ...
                <div className="space-y-1">
                   <label className="text-[9px] font-black text-zinc-600 uppercase ml-1">Directive Parameters</label>
                   <textarea name="desc" required placeholder="Outline operational goals..." className="w-full h-32 bg-zinc-950 border border-white/10 p-4 rounded-xl text-sm outline-none focus:border-indigo-500 transition-all text-white placeholder-zinc-800" />
                </div>
                <button type="submit" className="w-full py-4 bg-indigo-600 text-white font-black rounded-xl text-xs uppercase tracking-widest hover:bg-indigo-500 transition-all shadow-lg">Deploy Directive</button>
              </div>
            </form>
          </div>
        )}
        ...
        {activeTab === 'NOTICES' && (
          <div className="max-w-md mx-auto animate-in">
            <form onSubmit={(e) => {
              ...
              <h3 className="text-xl font-black text-center mb-6 text-white uppercase tracking-tighter">System Broadcaster</h3>
              <div className="space-y-4">
                <div className="space-y-1">
                   <label className="text-[9px] font-black text-zinc-600 uppercase ml-1">Broadcast Range</label>
                   <select name="target" className="w-full bg-zinc-950 border border-white/10 p-4 rounded-xl text-sm text-white appearance-none cursor-pointer focus:border-emerald-500">
                     <option value="ALL">Global Channel (Teamwide)</option>
                     <optgroup label="Direct Channel">
                        {users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                     </optgroup>
                   </select>
                </div>
                <div className="space-y-1">
                   <label className="text-[9px] font-black text-zinc-600 uppercase ml-1">Uplink Subject</label>
                   <input name="title" required placeholder="Header..." className="w-full bg-zinc-950 border border-white/10 p-4 rounded-xl text-sm outline-none focus:border-emerald-500 text-white placeholder-zinc-800" />
                </div>
                <div className="space-y-1">
                   <label className="text-[9px] font-black text-zinc-600 uppercase ml-1">Uplink Message</label>
                   <textarea name="content" required placeholder="Payload..." className="w-full h-32 bg-zinc-950 border border-white/10 p-4 rounded-xl text-sm outline-none focus:border-emerald-500 text-white placeholder-zinc-800" />
                </div>
                <button type="submit" className="w-full py-4 bg-emerald-600 text-black font-black rounded-xl text-xs uppercase tracking-widest hover:bg-emerald-500 transition-all shadow-lg">Execute Uplink</button>
              </div>
            </form>
          </div>
        )}
